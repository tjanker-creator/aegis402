// AEGIS402 — slot probe (VERIFY ONLY, never settles, moves no funds).
//
// Measures what the LIVE hosted facilitator will actually carry inside an x402
// payment group. Every row below was run against
// https://facilitator.goplausible.xyz on 2026-08-18 and the results are real.
// This is the de-risked skeleton for scripts/slot.mjs — swap facilitator("/verify")
// for verifyAndSettle() to make it move money.
...
async function build({ file="attestation.json", amount=20000, receiver=merchantAddr,
                       withConsent=true, withRegistry=true, badConsent=false, omitGuard=false, fill=0 }) {
...
await probe("guard-omitted",       { omitGuard: true });
await probe("ceiling-16",          { fill: 8 });

// Algorand itself caps a group at 16, so a 17th can only be presented as a
// 17-element payload array. The hosted facilitator refuses it by name.
{
  const b = await build({ fill: 8 });
  const g = [...b.paymentPayload.payload.paymentGroup];
  g.push(g[3]);
  const r = await facilitator("/verify",
    { ...b.paymentPayload, payload: { paymentGroup: g, paymentIndex: 15 } },
    b.paymentRequirements);
  console.log(`ceiling-17               size=17 isValid=${r.body?.isValid}`);
